...
(async () => {
  try {
    ...
    await page.goto('https://docs.aws.amazon.com/AWSJavaScriptSDK/v3/latest/');
    ...
    for (const link of clientLinks) {
      ...
      // Add a 5-second delay on each client-related page
    ...
    const extractedData = await page.evaluate((url, method, serviceDesc) => {
      ...
      function extractSectionData(sectionId) {
          ...
          return rows.map(row => {
              ...
              return { name: apiName, desc: apiDesc, data_type: dataType };
          });
          }
          return [];
      }
