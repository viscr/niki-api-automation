...
async function jiraCrawler(openApiUrl) {
  try {
    const parsedData = await fetchAndParseOpenAPI(openApiUrl);
    const apiSpecsFile = 'jira-app.json';
    const jsonString = JSON.stringify(parsedData, null, 2);
    console.log(jsonString);
    fs.writeFileSync(apiSpecsFile, jsonString);
    console.log('OpenAPI data saved to app-jira.json');
  } catch (error) {
