...
async function main() {
    const apiNames = fs.readFileSync('worked-apis.txt', 'utf-8').split('\n');
    const app = {
    ...
    try {
      const apiSpecsFile = 'app-config-test2.json';
      const jsonString = JSON.stringify(app, null, 2);
      fs.writeFileSync(apiSpecsFile, jsonString);
    } catch (e) {
      console.log(`Error writing to 'app-config-test2.json': ${e}`);
    }
}
